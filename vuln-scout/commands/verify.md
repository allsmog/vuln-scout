...
Joern output should resolve to one of:
- `VERIFIED`
- `FALSE_POSITIVE`
- `NEEDS_REVIEW`
- `NA_CPG`
...
Workflow:
1. Run Joern and collect the initial verification result.
2. Invoke `false-positive-verifier` with only the evidence bundle.
3. If the reviewer returns `CHANGES_REQUESTED`, revise the candidate, downgrade it to `hotspot`, or leave it as `needs_review`, then invoke a NEW cold reviewer.
4. Stop after 3 rounds maximum.
5. If the finding is still not settled after round 3, keep the final artifact verdict as `needs_review` and add `[REVIEWER NOTE: unresolved]` to `.claude/review-ledger.json`.
